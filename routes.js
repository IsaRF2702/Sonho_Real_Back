...
routes.put('/user/:id', async (req, res) => {
  try {
    ...
    if (user.length === 0) return res.status(404).json('Usuário não encontrado');
...
routes.post('/favorites', async (req, res) => {
  try {
    ...
    // Verifica se o imóvel já foi favoritado por esse usuário
    ...
    if (favorito.length > 0) {
      ...
      await sql`
        ...
        WHERE user_id = ${user_id} AND imovel_id = ${imovel_id}
      `;
      return res.status(200).json('Imóvel removido dos favoritos');
    } else {
      ...
      `;
      return res.status(200).json('Imóvel adicionado aos favoritos');
    }
  } catch (error) {
    console.error(error);
